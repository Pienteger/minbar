...
export function StoryList() {
    ...
    useEffect(() => {
        // Insert the user at the beginning of the stories array
        const currentUser = {
            id: user?.id.toString() || "0",
            username: "You",
            avatar: user?.profilePictureUrl || "/placeholder.svg",
            hasStory: false,
            isUser: true,
            stories: [],
        }

        userStories = [currentUser, ...userStories.filter((u) => u.id !== user?.id.toString())]
                        ...
                        >
                            <div className="rounded-full bg-background p-[2px]">
                                <div className="relative h-16 w-16 lg:h-20 lg:w-20 overflow-hidden rounded-full">
                                    <Image src={user.avatar || "/placeholder.svg"} alt={user.username} fill
                                           className="object-cover"/>
                                    {user.isUser && (
                                        <div
                                            className="absolute bottom-0 right-0 bg-gradient-to-r from-pink-500 to-orange-500 text-white rounded-full p-1">
                            ...
                            {user.stories.length > 1 && (
                                <div
                                    className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 bg-background rounded-full px-1.5 py-0.5 text-[10px] font-medium border border-primary/20">
                                    {user.stories.length}
                                </div>
                            )}
                        </div>
