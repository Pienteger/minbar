...
export function MosqueDiscovery({
    ...
    useEffect(() => {
        ...
        } else if (activeTab === "joined" && isAuthenticated && user?.id) {
            setSearchParams({
                ...params,
                userId: user.id,
            });
    ...
    const tabItems = [
        {value: "nearby", label: "Nearby", render: true},
        {value: "popular", label: "Popular", render: true},
        {value: "joined", label: "My Mosques", render: isAuthenticated},
    ];
        ...
        <div className="space-y-6">
            ...
            >
                <TabsList className="grid grid-cols-3 w-full max-w-md rounded-xl bg-muted/50 p-1">
                    {tabItems.filter((tab) => tab.render)
                        .map((tab) => (
                            <TabsTrigger
                                key={tab.value}
                                value={tab.value}
